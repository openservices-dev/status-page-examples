...
const TableWithCommits: React.FC<Props> = ({ projects }: Props) => {
  return (
    <>
      <Row>
            ...
            <tbody>
              {
                projects.map((project: Project, index: number) => (
                  <tr key={project.id}>
                    <td>{index + 1}</td>
                    <td>{project.name}</td>
                    <td>{project.commit?.branch}</td>
                    <td>{project.commit?.message} <small>({project.commit?.hash})</small></td>
...
export default TableWithCommits;
